...
class _TrainingStatsPageState extends State<TrainingStatsPage> {
  ...
  Widget _buildStatsView(BuildContext context, List<TrainingLog> logs) {
    ...
    for (var log in logs) {
      for (var round in log.sparringRounds) {
        totalSubsFor += round.submissionsFor;
        totalSubsAgainst += round.submissionsAgainst;
        totalSweeps += round.sweepsFor;
        totalPasses += round.passesFor;
      }
    }
    final List<_ChartData> sparringData = [
      _ChartData('Finalizações', totalSubsFor),
      _ChartData('Finalizado', totalSubsAgainst),
      _ChartData('Raspagens', totalSweeps),
      _ChartData('Passagens', totalPasses),
    ];
  ...
  Widget _buildSparringChart(
    ...
    final Map<SparringChartType, String> options = {
      SparringChartType.column: 'Colunas',
      SparringChartType.bar: 'Barras',
      SparringChartType.pie: 'Pizza',
    };

    Widget chart;
    switch (_sparringChartType) {
      case SparringChartType.pie:
    ...
    return _ChartCard(
      title: 'Estatísticas de Sparring',
      infoMessage:
          'Este gráfico exibe a soma total das suas ações de sparring registradas no diário.\n\nParâmetros:\n• Finalizações (a favor)\n• Finalizado (contra)\n• Raspagens (a favor)\n• Passagens de guarda (a favor)',
      chart: chart,
      options: options,
      currentValue: _sparringChartType,
      onChanged: (newValue) {
        if (newValue == null) return;
        setState(() => _sparringChartType = newValue);
        _saveChartPreference('sparringChart', newValue.name);
      },
    );
  }
  ...
  Widget _buildTechniquesChart(
    ...
    return _ChartCard(
      title: 'Técnicas Mais Treinadas',
      infoMessage:
          'Este gráfico exibe as 5 técnicas que você mais registrou em seus treinos, ajudando a visualizar seu foco.\n\nParâmetros:\n• Contagem de cada técnica registrada',
      chart: chart,
      options: options,
